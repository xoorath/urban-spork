 ...
 * 
 * 1) CPR_Reciever broadcasts to any listening game processes to announce that it's available for rendering.
 *          [game proc]: shuts its window down, or prevents its creation. begins streaming image data to the editor proc.
 * 2) CPR_Reciever receives an image data stream from the game processes.
 * 3) CPR_Reciever calls back it's listener (main.cpp) with a usable image to display.
...
// Let any listening game processes know that we're ready to receive a stream of image data.
